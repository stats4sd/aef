...
<div class="bg-white mt-4 shadow-xl px-20">
        ...
        @foreach ($studycase->photos as $photo)
            <div class="relative w-full sm:w-1/2 md:w-1/3 h-64">
                ...
                <div class="absolute bottom-0 w-full h-1/5 bg-dark-teal-70 flex items-center justify-center">
                    <p class="text-white text-center px-4 truncate max-w-full" style="max-width: calc(100% - 2rem);">{{ $photo->description }}</p>
                </div>
            </div>
        @endforeach
        </div>
    ...
    <div id="photoModal" class="fixed inset-0 hidden bg-black bg-opacity-80 items-center justify-center p-4" onclick="closePhotoModal()">
        <div class="relative w-auto max-w-3xl bg-white rounded-lg shadow-lg p-6 pt-8" onclick="stopPropagation()">
            <!-- Photo -->
            ...
            <!-- Close Button -->
            <button class="absolute top-0.5 right-2 text-gray-500 text-xl hover:text-gray-700 transition" onclick="closePhotoModal()">&times;</button>
        </div>
    </div>
