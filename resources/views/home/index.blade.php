...
@section('content')
    ...
    <div class="relative w-full h-[75vh]">
        ...
        <div class="absolute top-0 right-0 w-1/2 h-full bg-dark-teal text-white flex flex-col justify-center py-8 pl-16">
            <h1 class="text-6xl font-bold pr-40 mb-4">{{ t("GRASSROOTS EVIDENCE FOR AGROECOLOGY") }}</h1>
            <h2 class="text-2xl text-ochre mb-8 pt-6">{{ t("CASES AND EVIDENCE") }}</h2>
            <div class="flex space-x-4 pt-12">
                <a href="#cases" class="bg-ochre hover-effect text-white font-semibold py-2 px-6">{{ t("BROWSE CASES") }}</a>
                <a href="/about" class="bg-mint hover-effect text-white font-semibold py-2 px-6">{{ t("FIND OUT MORE") }}</a>
            </div>
        </div>
    </div>
